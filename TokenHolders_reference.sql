...
AND contract_address = {{token_address}} -- contract_address of the erc20 token 
UNION ALL
SELECT date_trunc('day',evt_block_time) as date,
tr."to" AS address,
(tr.value/1e18) AS amount
...
gs as (
SELECT date FROM UNNEST(sequence(date_trunc('day', CAST(NOW() - interval '1' year AS TIMESTAMP)),CAST(NOW() AS TIMESTAMP),interval '1' day)) as tbl(date)
),
